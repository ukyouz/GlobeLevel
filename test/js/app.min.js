!function () {
    ...
    function redraw_foreground(d3Elem, strPath, bShowRedPoint) {
        var projection = strPath.projection();
        d3Elem.append("path").datum({
            type: "Sphere"
        }).attr("class", "background").attr("d", strPath);
        d3Elem.append("path").datum(d3.geo.graticule().minorStep([90, 360]).extent([[-180, -0.05],[180, 0.05]])).attr("class", "graticule").attr("d", strPath);
        d3Elem.append("path").datum({
            type: "Sphere"
        }).attr("class", "foreground").attr("d", strPath).on("mousedown.grab", function () {
            var n;
    ...
    function n(t, n) {
        return d3.geo.orthographic().precision(.5).clipAngle(90)
    ...
    var e = 180 / Math.PI,
        ...
        c = -1,
        path_renderer;
    d3.select("body").attr("style", "background-size:" + (245/171)*100*490/(width/10) + "%");
    d3.selectAll("#map").data([
        n(width, height).scale(245).translate([width / 2, height / 2]),
        // d3.geo.eisenlohr().precision(.1).clipExtent([[-1, -1], [width + 1, height + 1]]).translate([width / 2, height]).scale(75).rotate([0, -30])
    ]).append("svg").attr("width", width).attr("height", height).each(function (n) {
        path_renderer = d3.geo.path().projection(n);
        a = d3.select(this).call(redraw_foreground, path_renderer, false);
        a.selectAll(".foreground").call(d3.geo.zoom().projection(n).scaleExtent([.7 * n.scale(), 10 * n.scale()]).on("zoom.redraw", function () {
            var scale = (d3.event.sourceEvent.scale / 171);
            d3.event.sourceEvent.preventDefault && d3.event.sourceEvent.preventDefault();
            // console.log(d3.event.sourceEvent.scale);
            d3.select("body").attr("style", "background-size:" + scale * 100 * 490 / (width / 10) + "%");
            // console.log(a);
            a.selectAll("path").attr("d", path_renderer);
            a.selectAll("text")
                // .filter(function (d) {
                //     return d3.polygonArea(d);
                // })
                .attr("x", function (d, i) {
                    return path_renderer.centroid(d)[0] || 0;
                })
                .attr("y", function(d, i) {
                    return path_renderer.centroid(d)[1] || 0;
                })
                .text(function (d, i) {
    ...
    //     .attr("x", function(d) {
        //         return strPath.centroid(d)[0] || 0;
        //     })
        //     .attr("y", function(d) {
            //         return strPath.centroid(d)[1] || 0;
            //     })
            //     .text(function(d) {
                //         return "t"
                //     })
                // var nodes = d3.selectAll("svg").insert("g", ".foreground").data(topojson.feature(n, n.objects.countries).features);
    d3.json("countries-110m.json", function (t, n) {
        // console.log("land", n)
        var land = d3.select("svg").datum({}).insert("g", ".foreground").attr("class", "land");
        land.selectAll("path").data(topojson.feature(n, n.objects.countries).features).enter()
            .append("path").attr("d", path_renderer).attr("class", function (d) {
                return "DIGITS"+d.id;
            })
        // d3.selectAll("svg").insert("path", ".foreground").datum(topojson.mesh(n, n.objects.land)).attr("class", "mesh");
        // d3.json("countries-110m.json", function (t, n) {
        var texts = d3.select("svg").datum({}).insert("g", ".foreground").attr("class", "text");
        // console.log("country", n.objects)
        texts.selectAll("path").data(topojson.feature(n, n.objects.countries).features).enter()
            .append("text")
            // .attr("text-anchor", "middle")
            ...
            .text(function (d) {
                return d3.polygonArea(d.geometry.coordinates[0])*(245/171) > 150 ? d.properties.name : "";
            })
        r.world();
        // });
    });
    ...
    window.onresize = function () {
        ...
        /* d3.selectAll("#map svg").data([
            n(width, height).scale(245).translate([width / 2, height / 2]),
            // d3.geo.eisenlohr().precision(.1).clipExtent([[-1, -1], [width + 1, height + 1]]).translate([width / 2, height]).scale(75).rotate([0, -30])
        ]).attr("width", width).attr("height", height).each(function (n) {
    ...
    /* function () {
        ...
        var u = d3.select("#comparison").append("svg").attr("width", a).attr("height", o).call(t, s, !0);
